# Competitor Analysis Prompt — Content Marketing (as a discipline feeding the core service)
...
competitor_type:
  ...
  service as its core offer
  "full_stack_niche" = a full-service company in the same industry offering content marketing as
  a distinct discipline feeding into the target service
  If missing or empty, include both types.

service: "content marketing" as the discipline feeding {SERVICE} (the target's core service,
inferred from {TARGET_URL} if not specified).
  (Only include companies where content marketing is treated as a genuine, distinct service that
  explicitly feeds into the target service — not companies that merely list "content" and the
  target service as separate, disconnected line items. Look for: a dedicated content marketing/
  content strategy service page, explicit language connecting content creation to the target
  service's outcomes, and ideally a real case study showing content driving results. A page
  where the target service is actually delivered without any content component does NOT count.)
...
- Find {LOCATION} companies that offer content marketing AND can demonstrate it explicitly feeds
  their delivery of the target service (not two disconnected offerings)
- Apply competitor_type and niche filtering if provided
- Exclude companies where content and the target service are just parallel items in a long
...
- Prefer companies that: have a dedicated content marketing/content strategy page, show explicit
  language linking content to the target service's outcomes, and cite a real case study
  ...
  "competitors": [
    {
      ...
      "offering_summary": "Runs a resource hub of gated reports and webinars feeding a newsletter, with every asset closing on a strategy-call CTA and case studies attached to the same service pages the content links into.",
      "similarity_score": 0.92,
      "avg_position": 12.5,
      "intersections": 450
    }
  ],
  "notes": "Returned 6 of 10. Four near-misses were excluded: two publish content with no route into any service page, one is an agency reselling a syndicated content product, and one had no content published since 2023."
}

Field rules:
- `content_marketing_page_url` — the page that best evidences the content-marketing programme — the service page, the
  resource hub, or the content library, whichever actually shows how content is produced and
  used.
- `offering_summary` — one or two sentences describing the **content-to-service pipeline observed**: what content
  types they publish, what the content asks the reader to do next, and how it connects to the
  services they sell. The client's own strategy is built against this pipeline, so describe the
